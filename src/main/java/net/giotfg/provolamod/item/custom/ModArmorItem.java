...
import net.giotfg.provolamod.item.ModArmorMaterials;
...
public class ModArmorItem extends ArmorItem {
    private static final Map<RegistryEntry<ArmorMaterial>, List<StatusEffectInstance>> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<RegistryEntry<ArmorMaterial>, List<StatusEffectInstance>>())
                    .put(ModArmorMaterials.GINO_ARMOR_MATERIAL,
...
//                    Aggiungere altri .put qui per altri custom (non vanilla) armor materials
                    .build();
...
//    inventoryTick viene chiamato ogni tick se il player ha un oggetto relativo alla classe corrente (ModArmorItem) nell'inventario
    @Override
    ...
    private boolean hasCorrectArmorOn(RegistryEntry<ArmorMaterial> material, PlayerEntity player) {
        for (ItemStack armorStack: player.getInventory().armor) {
            if(!(armorStack.getItem() instanceof ArmorItem)) { // Per evitare crash nel casting che avviene a r.84, nel caso ci sono oggetti come zucca o elytra che non sono ArmorItem
                return false;
            }
        }
